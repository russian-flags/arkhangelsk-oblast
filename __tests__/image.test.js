...
const defaultImage = createFlagImage("/flags/arkhangelsk.svg", "Флаг Архангельска");
assert.ok(defaultImage instanceof ImageCtor);
assert.equal(defaultImage.src, "/flags/arkhangelsk.svg");
assert.equal(defaultImage.alt, "Флаг Архангельска");
assert.equal(defaultImage.decoding, "async");
assert.equal(defaultImage.loading, "lazy");

const customImage = createFlagImage("/flags/arkhangelsk.svg", "Флаг Архангельска", {
  alt: "Флаг города Архангельска",
  decoding: "sync",
  loading: "eager",
...
assert.equal(customImage.alt, "Флаг города Архангельска");
...
assert.equal(customImage.title, "Архангельск");
assert.equal(customImage.id, "arkhangelsk-flag");
...
assert.equal(customImage.dataset.slug, "arkhangelsk");
